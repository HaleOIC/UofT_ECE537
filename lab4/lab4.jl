...
using Distributions, StatsBase, StatsPlots, LinearAlgebra, LaTeXStrings, PlutoUI, DSP, Plots
...
In the first part of the lab, we will be simulating low pass Guassian, and then we will use the spectrual analysis to have a better understanding of it. 
...
# ╔═╡ 64b646db-2339-49a5-901c-d1721c9373f4
plotly()
...
where $$X_k = X(kT)$$. and for this lab, we choose $X_k \sim N(0, 1)$ and the summation above matches the convolution $\sum_kX(kT)\delta(t - kT) * 2Bsinc(2Bt)$ for $B = 1 / 2T$, implying that $X(t)$ is a bandlimited (low-pass filtered) white noise signal with bandwidth $|f| < B$.
...
begin
	...
	# some useful functions to calculate the X(t)
	# calculate the X_k
	X_k(n) = [Normal(mu, sigma) for k = 1:1:n];

	# calculate the k_t - m to k_t + m
	k_t(t) = floor(Int, t / T);
	k_t(t, m) = max(k_t(t) + m, 1);

	# calculate the sum of target values
	X_t(X_k, t, T, m) = sum([X_k[k] * sinc(t / T - k) for k = k_t(t, -m):k_t(t, m)]);
end

# ╔═╡ 44fe2fa1-0b27-4f5f-a136-c203a2b9ceb4
begin
	# generate the X_k array and define the true function
	Xk  = rand.(X_k(20_000)); 
	Xt(t) = X_t(Xk, t, T, m);

	# generate the t series and calculate the corresponding function value
	ts  = 0:deltaT:(4000 * T);
	Xts = Xt.(ts)
end

# ╔═╡ f43d0781-0c63-48db-ba82-179023a27335
begin
	plot(ts, Xts, label="X(t)", xlabel="Time [s]", ylabel="Amplitude", title = "X(t)", color="red");
	plot!(; xlims=(ts[1],ts[end]));
end

# ╔═╡ 7ed2ab18-c3d6-4a9d-94ee-2a767eaf170e
md"
...
# ╔═╡ 20ffe069-926e-4f28-9a84-a42b056b06ba
begin
	...
	t_t(t, k) = (t - k_t(t)) + k * deltaT;
	Y(t, X_k, a, T, m) = sum([X_t(t - t_t(t, k), X_k, T, m) * exp(-a * t_t(t, k)) for k = 0:(20/deltaT)]) * deltaT;
	Y_t(t, a) = Y(t, Xk, a, T, m);
end

# ╔═╡ 9d45311e-15f4-41e6-968c-4d2449b1bfa8
begin
	Yt₁(t) = Y_t(t, alpha1);
...
# ╔═╡ 403138a3-d486-49fb-b79b-061c3ccbb188
md"
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "50a5679685153c742f595692486ae341fbe68ffd"
